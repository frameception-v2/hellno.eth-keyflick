    ...
    <div className="mt-4 p-4 border border-gray-300 dark:border-gray-700 rounded">
      ...
      {keys ? (
        <div className="space-y-1 text-sm break-all">
          ...
          {/* <p><strong>Public Key:</strong> {keys.publicKey}</p> */} {/* Removed Public Key */}
          <p className="text-red-600 dark:text-red-400"><strong>Private Key:</strong> {keys.privateKey}</p>
        </div>
      ) : (
        <p className="text-gray-500">Click the button above to generate {
            chainType === 'evm' ? 'EVM' : chainType === 'solana' ? 'Solana' : 'Bitcoin'
          } keys.</p>
      )}
    </div>
  );
    ...
    <div className="p-4 max-w-md mx-auto">
       ...
       {selectedChain === 'bitcoin' && (
         <div>
           ...
           <KeyDisplay title="Bitcoin (BTC)" keys={bitcoinKeys} chainType="bitcoin" />
         </div>
       )}
